...
def train_regressor ():
	...
	epsilon_set = np.asarray([0.01], dtype='float32')

	alpha_set = np.asarray([0,0.01], dtype='float32')

	mini_batch_sizes = np.asarray([48000], dtype='int') 

	epoch_lengths = np.asarray([20], dtype='int')
			...
			for total_epochs in epoch_lengths:
				...
				for mini_batch_size in mini_batch_sizes:
					...
					for epochs in range(0,total_epochs) :
						...
						indices = np.random.permutation(np.arange(np.shape(X_tr)[1]))
